...
export const sendConfirmationEmail = onDocumentCreated('appointments/{appointmentId}', async (event) => {
    try {
        ...
        <div style="background-color: #f5f5f5; padding: 15px; margin: 20px 0; border-radius: 5px;">
          ...
          ${
              appointmentData.vehicleInfo.brand
                  ? `<p><strong>Vehicle:</strong> ${appointmentData.vehicleInfo.brand} ${
                        appointmentData.vehicleInfo.model || ''
                    }</p>`
                  : ''
          }
...
${
    appointmentData.vehicleInfo.brand
        ? `Vehicle: ${appointmentData.vehicleInfo.brand} ${appointmentData.vehicleInfo.model || ''}`
        : ''
}
...
Price: ${appointmentData.price} BGN
      `.trim(),
            ...
            attendees: [{email: appointmentData.customerInfo.email}],
        };
...
export const syncGoogleCalendar = onDocumentCreated('appointments/{appointmentId}', async (event) => {
    try {
        const appointmentData = event.data?.data();
        if (!appointmentData) return;
